...
public class PaginationScanner implements SPI {
    ...
    public CompletableFuture<Void> run(ScanContext ctx) {
        return CompletableFuture.runAsync(() -> {
            int max = switch (ctx.preset) { case "fast" -> 8; case "aggressive" -> 30; default -> 16; };
            int tested = 0;
            List<String> hints = List.of("list","items","transactions","accounts","events","logs","orders","payments");
            for (String p : ctx.endpoints) {
                if (tested >= max) break;
                boolean candidate = false;
                String low = p.toLowerCase();
                for (String h : hints) if (low.contains(h)) { candidate = true; break; }
                ...
                long t50=0,t500=0,t5000=0; int c50=0,c500=0,c5000=0;
                try (Response r1 = ctx.http.request("GET", l50, null, null)) { t50 = r1.receivedResponseAtMillis()-r1.sentRequestAtMillis(); c50 = r1.code(); }
                catch (Exception ignored) {}
                try (Response r2 = ctx.http.request("GET", l500, null, null)) { t500 = r2.receivedResponseAtMillis()-r2.sentRequestAtMillis(); c500 = r2.code(); } catch (Exception ignored) {}
                try (Response r3 = ctx.http.request("GET", l5000, null, null)) {
                    t5000 = r3.receivedResponseAtMillis()-r3.sentRequestAtMillis(); c5000 = r3.code();
                    ...
                    if (slow || err) {
                        ...
                        si.endpoint = p;
                        si.method = "GET";
                        si.description = err? "Деградация/ошибки на больших выборках (429/5xx)":"Замедление при больших лимитах: limit/page";
                        si.evidence = "t50="+t50+"ms, t500="+t500+"ms, t5000="+t5000+"ms, code5000="+c5000;
