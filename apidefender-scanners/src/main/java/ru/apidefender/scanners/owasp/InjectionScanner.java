...
public class InjectionScanner implements SPI {
    ...
    private static final List<String> ERROR_SIGNS = List.of(
            "SQL syntax", "ORA-", "SQLSTATE", "MongoError", "PSQLException", "org.postgresql",
            "syntax error at or near",
            "unterminated quoted string", "invalid input syntax for type", "relation \"", "XPathException",
            "Stack trace");
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            for (String p : ctx.endpoints) {
                ...
                for (String payload : PAYLOADS) {
                    String url = base + (base.contains("?") ? "&" : "?") + "q="
                    ...
                    try (Response r = ctx.http.request("GET", url, null, null)) {
                        ...
                        boolean timing = (payload.toLowerCase().contains("pg_sleep")
                                || payload.toLowerCase().contains("sleep")) && dt > 1500;
                        if (error || timing) {
                            ...
                            si.endpoint = p;
                            si.method = "GET";
                            si.description = timing ? "Подозрение на time-based инъекцию"
                                    : "Подозрение на ошибку при инъекции";
                            ...
                            if (!"low".equalsIgnoreCase(ctx.exploitDepth)) {
                                ...
                                if ("med".equalsIgnoreCase(ctx.exploitDepth)
                                        || "high".equalsIgnoreCase(ctx.exploitDepth)) {
                            ...
                            if (notes.length() > 0)
                                si.description += "; Подтверждение: " + notes;
